

- [Read and Write User Input in Java](https://www.baeldung.com/java-console-input-output)

- [How to Create an Executable JAR with Maven](https://www.baeldung.com/executable-jar-with-maven)
- [Connecting to the Database](https://jdbc.postgresql.org/documentation/80/connect.html)
- [The try-with-resources Statement](https://docs.oracle.com/javase/tutorial/essential/exceptions/tryResourceClose.html)
- [Introduction to JDBC](https://www.baeldung.com/java-jdbc)
- [JDBC - Insert Records Example](https://www.tutorialspoint.com/jdbc/jdbc-insert-records.htm)
- [How to get the insert ID in JDBC?](https://stackoverflow.com/questions/1915166/how-to-get-the-insert-id-in-jdbc) 

- [Lombok Maven](https://projectlombok.org/setup/maven)